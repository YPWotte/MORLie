function [K] = K(phi,type)
%K Calculates derivative of exponential map
%   Maths found in "Derivative of the Expoential Map, Ethan Eade 2018"
%   phi is a 6 by 1 array of exponential coordinates
%   type defaults to 'normal', but 'inverse' may also be given to get K^(-1)(phi)
% Written by Y.P.Wotte, June 2021
...
else    % Series expansions to deal with numerical ill-conditioning
...
% b = @(th) (1-cos(th))./th.^2;
% c = @(th) (1 - a(th))./th.^2;
% e1 = @(th)(b(th)-2*c(th))./(2*a(th));
% e2 = @(th)(b(th) - a(th)/2)./(1-cos(th));
% f = @(th) (a(th) - 2*b(th))./th^2;
% g = @(th) (b(th) - 3*c(th))./th^2;
