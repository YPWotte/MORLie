%% Algorithm initialized with 1 rigid point cloud, but is able to handle arbitrary amounts.
...
B0 = 0*[1,0.5,0.2,0 ; 0.5,2,0.5,0; 0.2,0.5,3,0; 0,0,0,0]/T;                 % sheering and expanding component
...
SingValsTruncated1 = SingVals; SingValsTruncated1(trunc1+1:end,trunc1+1:end) = 0;
S_POD1 = U*SingValsTruncated1*(V');
SingValsTruncated2 = SingVals; SingValsTruncated2(trunc2+1:end,trunc2+1:end) = 0;
S_POD2 = U*SingValsTruncated2*(V');
SingValsTruncated3 = SingVals; SingValsTruncated3(trunc3+1:end,trunc3+1:end) = 0;
S_POD3 = U*SingValsTruncated3*(V');

Sk_POD1 = S_POD1(:,(k-1)*M+1:k*M);
Sk_POD2 = S_POD2(:,(k-1)*M+1:k*M);
Sk_POD3 = S_POD3(:,(k-1)*M+1:k*M);

Ett_POD1 = dist(Sk_POD1,Sk_GT,p_norm);
Ett_POD2 = dist(Sk_POD2,Sk_GT,p_norm);
Ett_POD3 = dist(Sk_POD3,Sk_GT,p_norm);
...
name = sprintf('POD_Error_%s_at_%s.svg',day,time);
name = strcat(FolderName,'/',name);
fig = figure();
T_tt = (0:M-1)*dt;
%plot(T_tt,Ett_param,T_tt,Ett_fft,T_tt,Ett_spline,T_tt,Ett_chip,T_tt,Ett_param_fin,T_tt,Ett_fft_fin,T_tt,Ett_spline_fin,T_tt,Ett_chip_fin,'LineWidth',2)
%plot(T_tt,Ett_spline,T_tt,Ett_chip,T_tt,Ett_spline_fin,T_tt,Ett_chip_fin,'LineWidth',2)
plot(T_tt,Ett_POD3,T_tt,Ett_POD1,T_tt,Ett_POD2,'LineWidth',2)


title('Full trajectory error for naive POD')
xlabel('Time in s')
ylabel('Error')
%legend('Parametric Fit','Fourier Fit', 'Spline Fit','Hermite Fit', 'Parametric Fit: Finite twist', 'Fourier Fit: Finite twist', 'Spline Fit: Finite twist','Hermite Fit: Finite twist')
%legend('Spline Fit','Hermite Fit','Spline Fit Finite','Hermite Fit Finite','Location','northwest')
lgd = legend('$N_j$','$5 N_j$','$9(N_j+1)+3$','Location','northwest');
title(lgd,'Basis elements');
fontsize(17,"points"); % fontsize(lgd,14,'points');
